...
from probables.blooms import NewBloomFilter, NewBloomFilterOnDisk
...
class TestNewBloomFilter(unittest.TestCase):
    """Test the default bloom filter implementation"""

    def test_bf_init(self):
        """test version information"""
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
    ...
    def test_bf_add(self):
        ...
        blm = NewBloomFilter(est_elements=20, false_positive_rate=0.05)
    ...
    def test_bf_union(self):
        ...
        blm.add("this is a test")
        blm.add("this is another test")
        blm2 = NewBloomFilter(est_elements=20, false_positive_rate=0.05)
    ...
    def test_bf_union_diff(self):
        ...
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
        blm.add("this is a test")
        blm2 = NewBloomFilter(est_elements=10, false_positive_rate=0.05, hash_function=different_hash)
    ...
    def test_bf_intersection(self):
        ...
        blm.add("this is a test")
        blm.add("this is another test")
        blm2 = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
    ...
    def test_bf_intersection_issue_57(self):
        ...
        blm = NewBloomFilter(est_elements=6425, false_positive_rate=0.001)
        blm.add("this is a test")
        blm.add("this is another test")
        blm2 = NewBloomFilter(est_elements=6425, false_positive_rate=0.001)
        blm2.add("this is another test")
        blm2.add("this is yet another test")
    ...
    def test_large_one_off_logl_compatibility(self):
        ...
        blm = NewBloomFilter(est_elements=16000000, false_positive_rate=0.0010)
    ...
    def test_bf_intersection_diff(self):
        ...
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
        blm.add("this is a test")
        blm2 = NewBloomFilter(est_elements=100, false_positive_rate=0.05)
    ...
    def test_bf_jaccard_invalid_msg(self):
        ...
        msg = "The parameter second must be of type NewBloomFilter or a NewBloomFilterOnDisk"
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
        blm.add("this is a test")
        try:
            blm.jaccard_index(1)
    ...
    def test_bf_load_hex(self):
        ...
        hex_val = "6da491461a6bba4d000000000000000a000000000000000a3d4ccccd"
        blm = NewBloomFilter(hex_string=hex_val)
    ...
    def test_bf_load_invalid_hex(self):
        ...
        self.assertRaises(InitializationError, lambda: NewBloomFilter(hex_string=hex_val))
    ...
    def test_bf_bytes(self):
        """test exporting NewBloomFilter to bytes"""
        md5_val = "8d27e30e1c5875b0edcf7413c7bdb221"
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
        blm.add("this is a test")
        b = bytes(blm)
    ...
    def test_bf_frombytes(self):
        """test loading NewBloomFilter from bytes"""
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05)
        blm.add("this is a test")
        bytes_out = bytes(blm)

        blm2 = NewBloomFilter.frombytes(bytes_out)
    ...
    def test_bf_load_file(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm.export(fobj.name)
            blm2 = NewBloomFilter(filepath=fobj.name)
    ...
    def test_bf_load_invalid_file(self):
        ...
        self.assertRaises(InitializationError, lambda: NewBloomFilter(filepath=filename))
    ...
    def test_bf_invalid_params_msg(self):
        ...
        try:
            NewBloomFilter(filepath=filename)
    ...
    def test_invalid_fpr(self):
        ...
        def runner():
            """runner"""
            NewBloomFilter(est_elements=100, false_positive_rate=1.1)
    ...
    def test_invalid_estimated_els(self):
        ...
        def runner():
            """runner"""
            NewBloomFilter(est_elements=0, false_positive_rate=0.1)
    ...
    def test_invalid_number_hashes(self):
        ...
        def runner():
            """runner"""
            NewBloomFilter(est_elements=10, false_positive_rate=0.999)
    ...
    def test_bf_clear(self):
        ...
        for idx in range(blm.bloom_length):
            self.assertEqual(blm.bloom[idx], 0)
    ...
    def test_bf_use_different_hash(self):
        ...
        blm = NewBloomFilter(est_elements=10, false_positive_rate=0.05, hash_function=my_hash)
        self.assertEqual(blm.elements_added, 0)
        blm.add("this is a test")
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm.export(fobj.name)
...
class TestNewBloomFilterOnDisk(unittest.TestCase):
    ...
    def test_bfod_ee(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blmd = NewBloomFilterOnDisk(fobj.name, 20, 0.05)
    ...
    def test_bfod_check(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm = NewBloomFilterOnDisk(fobj.name, 10, 0.05)
            blm.add("this is a test")
            blm.add("this is another test")
            self.assertEqual(blm.check("this is a test"), True)
            self.assertEqual(blm.check("this is another test"), True)
    ...
    def test_bfod_union(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm = NewBloomFilterOnDisk(fobj.name, 20, 0.05)
            blm.add("this is a test")
            blm.add("this is another test")
            blm2 = NewBloomFilter(20, 0.05)
            blm2.add("this is yet another test")
    ...
    def test_bfod_load_on_disk(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm = NewBloomFilter(10, 0.05)
            blm.add("this is a test")
            blm.export(fobj.name)

            blmd = NewBloomFilterOnDisk(fobj.name)
    ...
    def test_bfod_load_invalid_file(self):
        ...
        filename = "invalid.blm"
        self.assertRaises(InitializationError, lambda: NewBloomFilterOnDisk(filepath=filename))
    ...
    def test_bfod_invalid_params_msg(self):
        ...
        try:
            NewBloomFilterOnDisk(filepath=filename)
        except InitializationError as ex:
            self.assertEqual(str(ex), msg)
        else:
            self.assertEqual(True, False)
    ...
    def test_bfod_frombytes(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            ...
            blm.add("this is a test")
            bytes_out = bytes(blm)
        self.assertRaises(NotSupportedError, lambda: NewBloomFilterOnDisk.frombytes(bytes_out))
    ...
    def test_bfod_frombytes_msg(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            ...
            bytes_out = bytes(blm)
        try:
            NewBloomFilterOnDisk.frombytes(bytes_out)
    ...
    def test_bfod_export_hex(self):
        ...
        hex_val = "6da491461a6bba4d000000000000000a000000000000000a3d4ccccd"
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm = NewBloomFilterOnDisk(fobj.name, est_elements=10, false_positive_rate=0.05)
            for i in range(0, 10):
                tmp = "this is a test {0}".format(i)
                blm.add(tmp)
            hex_out = blm.export_hex()
    ...
    def test_bfod_load_hex(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            ...
            self.assertRaises(
                NotSupportedError,
                lambda: NewBloomFilterOnDisk(filepath=fobj.name, hex_string=hex_val),
            )
    ...
    def test_bfod_load_hex_msg(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            try:
                NewBloomFilterOnDisk(filepath=fobj.name, hex_string=hex_val)
            except NotSupportedError as ex:
    ...
    def test_bfod_clear(self):
        ...
        with NamedTemporaryFile(dir=os.getcwd(), suffix=".blm", delete=DELETE_TEMP_FILES) as fobj:
            blm = NewBloomFilterOnDisk(filepath=fobj.name, est_elements=10, false_positive_rate=0.05)
            self.assertEqual(blm.elements_added, 0)
            for i in range(0, 10):
                tmp = "this is a test {0}".format(i)
                blm.add(tmp)
